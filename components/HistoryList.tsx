  ...
  return (
    <div className="flex flex-col space-y-3 pb-24">
      ...
      {items.map((item) => (
        <div 
          key={item.id} 
          className={`relative group bg-white rounded-xl shadow-sm border-l-4 p-4 transition-all active:scale-[0.98] ${
            item.duplicate ? 'border-amber-500 bg-amber-50' : 'border-blue-500'
          }`}
        >
          {item.duplicate && (
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            {item.sku && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-gray-400 w-8">SKU</span>
                <span className={`text-sm font-mono break-all font-medium ${item.duplicate_fields.includes('sku') ? 'text-amber-700 underline decoration-amber-300' : 'text-gray-800'}`}>
                  {item.sku}
                </span>
              </div>
            )}
            {item.mac && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-gray-400 w-8">MAC</span>
                <span className={`text-sm font-mono break-all font-medium ${item.duplicate_fields.includes('mac') ? 'text-amber-700 underline decoration-amber-300' : 'text-gray-800'}`}>
                  {item.mac}
                </span>
              </div>
            )}
